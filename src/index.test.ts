...
${await Bun.file("web/src/App.vue").text()}`;
  ...
  it("defines focused runtime history, live events, and redacted event routes", async () => {
    ...
    expect(app).toContain('const selectedRunEvent = ref<TerraformRunEvent | null>(null);');
    expect(app).toContain('selectedRunEvent.value = null;\n    return;');
    ...
    expect(app).toContain('const runHistoryTree = computed<RuntimeHistoryTreeNode[]>(() => {');
    expect(app).toContain('<el-tree v-else :data="runHistoryTree"');
    expect(app).toContain('@node-click="selectRunHistoryNode"');
    expect(app).toContain('t("panel.runHistory")');
    expect(app).toContain('<el-dialog v-model="runtimeRunDialogVisible"');
    expect(app).toContain('v-loading="activeRunLoading"');

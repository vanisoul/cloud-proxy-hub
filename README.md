...
- `/config/apis/<provider-type>/<api-id>/secret.json`: published API variables and runtime output token hash.
...
10. `POST /api/apis/:apiId/runtime-output-token`
11. `GET /api/runtime/:apiId/outputs/:outputName`

Set `Authorization: Bearer <ADMIN_API_KEY>` for all API routes.

`GET /api/runtime/:apiId/outputs/:outputName` is the read-only runtime output surface for consumers. It accepts the per-publication runtime output token with `X-API-Key: <runtime-output-token>` or `Authorization: Bearer <runtime-output-token>`. The route returns one top-level Terraform output value from the latest successful deploy snapshot and never returns admin run metadata. Successful delete disables the current runtime output.

`POST /api/apis/:apiId/runtime-output-token` rotates the runtime output token and requires `Authorization: Bearer <ADMIN_API_KEY>`.
...
- Runtime output tokens are independent from `ADMIN_API_KEY`; only token hashes are stored, and raw tokens are returned only when an API is created or rotated.
